...
The backend is configured in `versions.tf` to use the bucket `notes-app-tfstate` with prefix `terraform/state`. Create it once before the first `terraform init`:
```sh
gsutil mb -p bootcamp-478214 -l us-central1 gs://notes-app-tfstate
```
...
```bash
gcloud projects add-gcloud functions describe -policy-binding $PROJECT_ID \
